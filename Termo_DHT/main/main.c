/* 
  ESP-01
  1. Измерерние температуры и влажности датчик DHT
  2. Сохранение показаний датчика в кольцевой буфер
  3. Управление реле
  4. Синхронизация времени по NTP
   Copyright (c) 2021, Antonov Technologies
   All rights reserved.
*/
static char Version[] = "\"2.05_13.09.2021\"";
...
esp_err_t power_get_handler(httpd_req_t *req)
{
    ...
    for (int n = 0; n < 100; n++)
    {
        ...
        for (int nPac = 0; nPac < 10; nPac++)
        {
            ...
            sprintf(begBuf, out1, p_buf[dataZ].now, p_buf[dataZ].humidity, p_buf[dataZ].temperature);
...
esp_err_t statusDHT_get_handler(httpd_req_t *req)
{
    ...
    if (dht_data.now)
    {
        const char *out1 = "now:%d,humidity:%d,temperature:%d";
        sprintf(out, out1, dht_data.now, dht_data.humidity, dht_data.temperature);
    }
    else {
        const char *out1 = "not data";
        sprintf(out, out1);
    }
...
void DHT_temperature_task(void *arg)
{
    ...
    while (1)
    {
        //int humidity = 0;
        //int temperature = 0;
        if (dht_read_data(DHT_TYPE_DHT22, DHT_GPIO, &dht_data.humidity, &dht_data.temperature) == ESP_OK)
        {
            time_t now;
            time(&now);
            dht_data.now = now;
...
static void Save_DHT(void *arg)
{
    ...
    while (1)
    {
        if (p_buf[p_dataZ].humidity != dht_data.humidity || p_buf[p_dataZ].temperature != dht_data.temperature)
        {
            if (time_start)
            {
                p_dataZ++;
                ...
                p_buf[p_dataZ].humidity = dht_data.humidity;
                p_buf[p_dataZ].temperature = dht_data.temperature;
