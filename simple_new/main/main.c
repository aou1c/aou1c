/* 
  ...
  EEPROM 
   ...
   All rights reserved.
*/
static char Version[] = "\"2.04_10.08.2021\"";

//#include </home/aou/esp/simple_new/main/include/dht/dht.h>
//#include </home/aou/esp/simple_new/main/include/dht/dht.c>
...
static int resultssensor = 0;

//#include "ds18b20.h"
//const int DS_PIN = D6; //GPIO where you connected ds18b20
...
static data_Z p_buf[SizeBuffer]; //Кольцевой буфер

static data_DHT DHT; //Текущее значение

static data_DHT p_buf_DHT[SizeBuffer]; //Кольцевой буфер

static int p_dataZ = 0;
static int p_dataDHT = 0;
...
esp_err_t power_get_handler(httpd_req_t *req)
{
    ...
    {
        begBuf = buf;
        for (int nPac = 0; nPac < 10; nPac++)
        {
            dataZ++;
            if (dataZ > SizeBuffer)
                dataZ = 0;

            sprintf(begBuf, out1, p_buf[dataZ].now, p_buf[dataZ].power, p_buf[dataZ].voltage);
            begBuf += strlen(begBuf);
        }
...
esp_err_t powerDHT_get_handler(httpd_req_t *req)
{
    int remaining = req->content_len;
    ...
    for (int n = 0; n < 100; n++)
    {
        ...
        for (int nPac = 0; nPac < 10; nPac++)
        {
            ...
            sprintf(begBuf, out1, p_buf_DHT[dataZ].now, p_buf_DHT[dataZ].humidity, p_buf_DHT[dataZ].temperature);
            begBuf += strlen(begBuf);
        }
        httpd_resp_send_chunk(req, buf, strlen(buf));
    }
...
esp_err_t statusDHT_get_handler(httpd_req_t *req)
{

    char *out = NULL;
    out = malloc(50);

    const char *out1 = "humidity:%d,temperature:%d";
    sprintf(out, out1, DHT.humidity, DHT.temperature);
...
esp_err_t  status_get_handler(httpd_req_t * req)
{
    ...
    if (is_param_status_Z(req))
    {
        ...
        sprintf(out, out1, pin_out, countSave, sumZ_count[0], sumZ_count[1], sumZ_count[4], sumZ_count[5], now, sumZ_count[2], sumZ_count[3], resultssensor, _currentValues.voltage, _currentValues.current, _currentValues.power, _currentValues.energy, _currentValues.frequeny, _currentValues.pf, time_start, Version);
    }
    else
    {
        const char *out1 = "{\t\"OUT\":\t%s,\n\t\"CountImp\":\t%d,\n\t\"CountDay\":\t%d, \n\t\"CountNight\":\t%d,\n\t\"CountDayPrev\":\t%d,\t\n\t\"CountNightPrev\":\t%d,\n\t\"DATE_TIME\":\t%d,\n\t\"CountDaySum\":\t%d,\n\t\"CountNightSum\":\t%d,\n\t\"Temperature\":\t%d,\n\t\"V\":\t%f,\n\t\"I\":\t%f,\n\t\"P\":\t%f,\n\t\"E\":\t%f,\n\t\"F\":\t%f,\n\t\"K\":\t%f\n}";
        sprintf(out, out1, pin_out, countSave, sum_count[0], sum_count[1], sum_count[4], sum_count[5], now, sum_count[2], sum_count[3], resultssensor, _currentValues.voltage, _currentValues.current, _currentValues.power, _currentValues.energy, _currentValues.frequeny, _currentValues.pf);
    }
...
httpd_uri_t powerDHT = {
    .uri = "/powerDHT",
    .method = HTTP_GET,
    .handler = powerDHT_get_handler,
    /* Let's pass response string in user
     * context to demonstrate it's usage */
    .user_ctx = NULL};


httpd_uri_t statusDHT = {
    .uri = "/statusDHT",
    .method = HTTP_GET,
    .handler = statusDHT_get_handler,
    /* Let's pass response string in user
     * context to demonstrate it's usage */
    .user_ctx = NULL};
            ...
            for (;;)
            {
                ...
                else
                {
                    for (;;)
                    {
                        ...
                        {
                            resultssensor = (int)(results[i]*100);
...
httpd_handle_t start_webserver(void)
{
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_register_uri_handler(server, &statusDHT);
        httpd_register_uri_handler(server, &powerDHT);
        //максимальное количество httpd_register_uri_handler = 8 !!!
...
#define DHT_GPIO D6 // D1 pin
void DHT_temperature_task(void *arg)
{
    ...
    while (1)
    {
        //int humidity = 0;
        //int temperature = 0;
        if (dht_read_data(DHT_TYPE_DHT11, DHT_GPIO, &DHT.humidity, &DHT.temperature) == ESP_OK)
        {
            time_t now;
            time(&now);
            DHT.now = now;
...
static void Save_DHT(void *arg)
{
    ...
    while (1)
    {
        if (p_buf_DHT[p_dataDHT].humidity != DHT.humidity || p_buf_DHT[p_dataDHT].temperature != DHT.temperature)
        {
            if (time_start)
            {
                p_dataDHT++;
                if (p_dataDHT > SizeBuffer)
                {
                    p_dataDHT = 0;
                    ESP_LOGI(TAG, "begin dataZ: %d \n", p_dataDHT);
                }
                time_t now;
                time(&now);
                p_buf_DHT[p_dataDHT].now = now;
                p_buf_DHT[p_dataDHT].humidity = DHT.humidity;
                p_buf_DHT[p_dataDHT].temperature = DHT.temperature;
                ESP_LOGI(TAG, "Save DHT: %d \n", DHT.temperature);
            }
            else
            {
...
static void Save_DS18B20(void *arg)
{
    ...
    while (1)
    {
        if (p_buf_DHT[p_dataDHT].temperature != resultssensor)
        {
            if (time_start)
            {
                p_dataDHT++;
                ...
                time(&now);
                p_buf_DHT[p_dataDHT].now = now;
                p_buf_DHT[p_dataDHT].humidity = 0;
                p_buf_DHT[p_dataDHT].temperature = resultssensor;
                ESP_LOGI(TAG, "Save DHT: %d \n", resultssensor);
            }
            else
            {
...
//        initialise_wifi(&server, true);
        ...
        xTaskCreate(&Save_DS18B20, "Save_DS18B20", 2048, NULL, 10, NULL);
        ...
        //xTaskCreate(&DHT_temperature_task, "temperature task", 2048, NULL, tskIDLE_PRIORITY, NULL);
        //xTaskCreate(&Save_DHT, "Save_DHT", 2048, NULL, 10, NULL);
